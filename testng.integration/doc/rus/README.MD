...
из перечисленных выше элементов (указывается как строка, в которой элементы разделены запятыми)
...
- Если все наборы тестов (xml suits) выполняются в один поток, то достаточно
```properties
...
- Если среди наборов тестов (xml suits) есть те, в которых используется много-поточность на уровне тестов, классов или тест-инстансов, то 
можно добавить
```properties
...
- Если среди наборов тестов (xml suits) есть те, в которых используется много-поточность на уровне выполняемых методов, то 
можно добавить
```properties
...
public class DependencyInjectionTest extends BaseTestNgTest {
    ...
    private int c;  //во всех тестах поле 'c' должно быть равно 2
    private Integer d;
