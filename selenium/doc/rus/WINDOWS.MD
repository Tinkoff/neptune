...
много или мало, тогда это время можно изменить, поменяв [настройку времени на поиск окна](./SETTINGS.MD#Поиск-окна).
...
public class Tezzt {
    ...
    public void test() {
        ...
        //Можно заранее найти окно/вкладку
        var w = inBrowser().get(window());
        var t3 = inBrowser().windowTitle(w); //и получить ссылку на загруженную страницу
    }
}
```
