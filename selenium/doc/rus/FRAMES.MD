...
public class Tezzt {
    ...
    public void test() {
        ...
        boolean isAlertPresent = inBrowser().presenceOf(frame(1), //Ожидание наличия фрейма. 
                "Фрейм не появился"); //Текст ошибки, если фрейм не появился
        // вернется true если фрейм есть, если нет - 
        // будет выброшено исключение с текстом 'Фрейм не появился'
