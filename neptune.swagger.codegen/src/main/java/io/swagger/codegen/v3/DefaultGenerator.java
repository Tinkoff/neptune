...
public class DefaultGenerator extends AbstractGenerator implements Generator {
    ...
    private static String sanitizeOperationId(String operationId) {
        // input[] => input
        var newOperationId = operationId.replaceAll("\\[]", "");

        // input[a][b] => input_a_b
        newOperationId = newOperationId.replaceAll("\\[", "");
        newOperationId = newOperationId.replaceAll("]", "");
        ...
        // input.newOperationId => input_name
        newOperationId = newOperationId.replaceAll("\\.", "");

        // input-newOperationId => input_name
        newOperationId = newOperationId.replaceAll("-", "");

        // input newOperationId and age => input_name_and_age
        newOperationId = newOperationId.replaceAll(" ", "");

        return newOperationId;
    }

    private static String[] getExpandedArray(String[] parts, String splitter) {
        var resultList = new LinkedList<String>();

        for (var part : parts) {
            var newParts = part.split(splitter);
            for (var newPart : newParts) {
                resultList.addLast(newPart);
            }
        }
    ...
    private static String generateOperationId(Operation operation, String path, String httpMethod) {
        ...
        if (isBlank(operationId)) {
            ...
            if (parts.length > 1) {
                parts = getExpandedArray(parts, "-");
                parts = getExpandedArray(parts, "_");
            }
            ...
            operationId = sanitizeOperationId(builder.toString());
        }
        return operationId;
    }
        ...
        */
        ...
        for (Tag tag : tags) {
            if (operation.getOperationId() == null) {
                var newId = generateOperationId(operation, resourcePath, httpMethod);
                operation.setOperationId(newId);
            }
            processOperation(config, tags, tag, resourcePath, httpMethod, operation, schemas, operations, securitySchemes, globalSecurities, openAPI);
        }
    }

    private void processOperation(CodegenConfig config,
                                  List<Tag> tags,
                                  Tag tag,
                                  String resourcePath,
                                  String httpMethod,
                                  ...
                                  List<SecurityRequirement> globalSecurities,
                                  OpenAPI openAPI) {
        if (operation.getRequestBody() == null) {
            handleOperation(config, tags, tag, resourcePath, httpMethod, operation, schemas, operations, securitySchemes, globalSecurities, openAPI);
        } else {
            ...
            if (content == null) {
                ...
                var schema = new ArrayList<>(content.values()).get(0).getSchema();

                if (!Schema.class.equals(schema.getClass())) {
                    handleOperation(config, tags, tag, resourcePath, httpMethod, operation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                    return;
                }
                ...
                var refComponents = schema.get$ref().split("/");
                var name = refComponents[refComponents.length - 1];
                var foundSchema = schemas.get(name);
                ...
                if (!ComposedSchema.class.equals(foundSchema.getClass())) {
                    handleOperation(config, tags, tag, resourcePath, httpMethod, operation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                    return;
                }

                var composedSchema = (ComposedSchema) foundSchema;
                if (composedSchema.getAllOf() != null) {
                    composedSchema.getAllOf().forEach(schema1 -> {
                        var newMediaType = new MediaType();
                        newMediaType.schema(schema1);
                        var newOperation = newOperation(operation.getOperationId(), operation, body, new ArrayList<>(content.keySet()).get(0), newMediaType);
                        processOperation(config, tags, tag, resourcePath, httpMethod, newOperation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                    });
                    return;
                }

                if (composedSchema.getOneOf() != null) {
                    composedSchema.getOneOf().forEach(schema1 -> {
                        var newMediaType = new MediaType();
                        newMediaType.schema(schema1);
                        var newOperation = newOperation(operation.getOperationId(), operation, body, new ArrayList<>(content.keySet()).get(0), newMediaType);
                        processOperation(config, tags, tag, resourcePath, httpMethod, newOperation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                ...
                if (composedSchema.getAnyOf() != null) {
                    composedSchema.getAnyOf().forEach(schema1 -> {
                        var newMediaType = new MediaType();
                        newMediaType.schema(schema1);
                        var newOperation = newOperation(operation.getOperationId(), operation, body, new ArrayList<>(content.keySet()).get(0), newMediaType);
                        processOperation(config, tags, tag, resourcePath, httpMethod, newOperation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                ...
                content.forEach((s, mediaType) -> {
                    var newOperation = newOperation(counter.count > 1 ? operation.getOperationId() + counter.count : operation.getOperationId(), operation, body, s, mediaType);
                    processOperation(config, tags, tag, resourcePath, httpMethod, newOperation, schemas, operations, securitySchemes, globalSecurities, openAPI);
                    counter.count++;
                });
            }
    ...
    private Operation newOperation(String operationId, Operation operation, RequestBody body, String content, MediaType mediaType) {
        var newOperation = new Operation();
        newOperation.setOperationId(operationId);
        newOperation.setSecurity(operation.getSecurity());
        newOperation.setCallbacks(operation.getCallbacks());
        newOperation.setDeprecated(operation.getDeprecated());
        newOperation.setDescription(operation.getDescription());
        newOperation.setExtensions(operation.getExtensions());
        newOperation.setExternalDocs(operation.getExternalDocs());
        newOperation.setParameters(operation.getParameters());
        newOperation.setResponses(operation.getResponses());
        newOperation.setServers(operation.getServers());
        newOperation.setSummary(operation.getSummary());
        newOperation.setTags(operation.getTags());

        var newBody = new RequestBody();
        newBody.set$ref(body.get$ref());
        newBody.setDescription(body.getDescription());
        newBody.setExtensions(body.getExtensions());
        newBody.setRequired(body.getRequired());

        var newContent = new Content();
        newContent.put(content, mediaType);

        newBody.setContent(newContent);
        newOperation.setRequestBody(newBody);
        return newOperation;
    }

    private void handleOperation(CodegenConfig config,
                                 List<Tag> tags,
                                 Tag tag,
                                 String resourcePath,
                                 String httpMethod,
