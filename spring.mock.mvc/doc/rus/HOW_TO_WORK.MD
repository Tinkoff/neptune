...
public class MyTest {
    ...
    private MockMvc mockMvc; //при котором данное поле будет 
    //заполнено на момент начала теста

    @Test
    public void myTest() {
        MockHttpServletResponse response = mockMvcGet(response(
                post("/some/path/and?query=params") //запрос
                        .contentType(APPLICATION_JSON) //и его параметры 
                        .contentType("{\"someString 1\"}"))
                .expect(status().isOk()) //тут можно указать параметры ожидаемого ответа
                .expect(content().string(containsString("SUCCESS"))) //проверка пройдет по всем 
...
Так же можно обойтись бех [механизма внедрения зависимостей Spring](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-dependencies)
...
public class MyTest {
    ...
    public void myTest() {
        var mockMvc = MockMvcBuilders
                .webAppContextSetup(/*параметры*/)
                /*указываем прочие параметры*/
                .build();
        
        var response = mockMvcGet(response(
                mockMvc, //явная передача инстанса MockMvc
                post("/some/path/and?query=params") 
...
public class MyTest {
    ...
    public void myTest() {
        ...
        DtoList body = mockMvcGet(response(
                post("/some/path/and?query=params") 
                        .contentType(APPLICATION_JSON) 
                        .contentType("{\"someString 1\"}"))
                .expect(status().isOk())
                .expect(content().string(containsString("SUCCESS")))
                .thenGetIterable("List of dto's", //описание списка, который следует получить
                        transformer, //указываем с помощью какого объекта происходит десериализация
...
public class MyTest {
    ...
    public void myTest() {
        ...
        List<Object> value = mockMvcGet(response(
                ...
                .expect(status().isOk())
                .expect(content().string(containsString("SUCCESS")))
                .thenGetIterable("Value of the field 'listValue'", //описание списка, который следует получить
                        transformer, //указываем с помощью какого объекта происходит десериализация
                        Dto.class, //указываем, в объект какого класса должно быть десериализовано тело ответа
...
public class MyTest {
    ...
    public void myTest() {
        ...
        Object[] value = mockMvcGet(response(
                ...
                .expect(status().isOk())
                .expect(content().string(containsString("SUCCESS")))
                .thenGetIterable("Value of the field 'arrayValue'", //описание массива, который следует получить
                        transformer, //указываем с помощью какого объекта происходит десериализация
                        new TypeReference<GenericDto<SomeType>>() {}, //указываем, в объект какого типа должно быть 
                        // десериализовано тело ответа
