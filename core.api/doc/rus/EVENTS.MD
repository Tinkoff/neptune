...
- В файл `ru.tinkoff.qa.neptune.core.api.event.firing.EventLogger` внести запись
  `org.my.pack.MyEventLogger`
...
Ниже пример того, как описать создание аттачей. Для этого можно:
...
```
    ...
    +---- services
             |
             +----ru.tinkoff.qa.neptune.core.api.event.firing.Captor
```

- В файл `ru.tinkoff.qa.neptune.core.api.event.firing.Captor` внести запись
  `org.my.pack.MyCaptor`
