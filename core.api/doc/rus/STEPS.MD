...
- [Шаги](#Шаги)
    ...
    - [Шаги, которые возвращают результат](#Шаги-которые-возвращают-результат)
        - [Шаг, который возвращает объект (`java.lang.Object` и его наследники)](#Шаг-который-возвращает-объект-javalangObject-и-его-наследники)
          - [Шаг, который возвращает объект](#Шаг-который-возвращает-объект)
          - [Шаг, который возвращает объект, соответствующий определенным критериям](#Шаг-который-возвращает-объект-соответствующий-определенным-критериям)
        ...
        - [Шаг, который возвращает List](#Шаг-который-возвращает-List)
    ...
    - [Критерии](#Критерии)
        - [Объединение критериев в один логическим И](#Объединение-критериев-в-один-логическим-И)
        - [Объединение критериев в один инклюзивным ИЛИ](#Объединение-критериев-в-один-инклюзивным-ИЛИ)
        - [Объединение критериев в один исключающим ИЛИ](#Объединение-критериев-в-один-исключающим-ИЛИ)
        - [Инверсия критериев](#Инверсия-критериев)
        ...
        //возвращают ссылку на объект, от которого ини были вызваны
{
    ...
    // Здесь описывается Map<String, String>, в котором ключ - название параметра, значение - значение параметра.
    // Не всегда параметры можно передать до начала выполнения шага. Некоторые значения могут получиться в процессе 
    // выполнения шага. Если эти значения доступны и есть необходимость отобразить их в отчете / в логе выполнения 
    // теста, то можно перекрыть данный метод.
    @Override
...
public class SomeTest {
    ...
    public void test() {
        ...
        // и в этом случае нужно выбросить исключение и уронить тест. 
...
Подходит для ситуаций, когда результат выполнения шага рассматривается как один объект.

[к оглавлению документа](#Оглавление) [к списку документов](README.MD#Оглавление)

##### Шаг, который возвращает объект. 
 ...
 * "посредника" не предусматривается.
 */
@ThrowWhenNoData(toThrow = IllegalStateException.class, startDescription = "Can't get:")
public class MyGetStepSupplier<T> extends SequentialGetStepSupplier.GetSimpleStepSupplier<
        MyTestContext,
        T,
        MyGetStepSupplier<T>>
{
...
##### Шаг, который возвращает объект, соответствующий определенным критериям.
        ...
        MyGetStepSupplier<T, M>>
{
    ...
    // перекрывать необязательно, уже писали ранее. Для наглядности
    @Override
    protected Map<String, String> additionalParameters() {
        //Возврат дополнительных динамических параметров, которые 
        // были рассчитаны уже в процессе выполнения шага
...
Примеры ниже работают по тем же принципам, что и примеры выше. Поэтому фокус на принципиальных отличиях.
        ...
        MyGetStepSupplier<T>>
{
    ...
    protected MyGetStepSupplier() {
        this(myTestContext -> {
            var o = myTestContext.getSomeResource();
            ///дальнейшие вычисления
            return //возврат КОЛЛЕКЦИИ или какого либо другого Iterable<T>
        });
    }
...
#### Шаг, который возвращает List

Подходит для ситуаций, когда результат необходимо вернуть в виде объекта `java.util.List`.
...
/**
 * Так описывается шаг, возвращающий java.util.List.
 * 
 * В данном случае наличие "посредника" не предусматривается, т.е. коллекция получается напрямую от входного параметра.
 * Критерии проверяют на соответствие все объекты, входящие в коллекцию, и если такие объекты были найдены - они возвращаются
 * в виде java.util.List (не модифицируемого).
 * 
 ...
 * вернется пустой List
 */
@ThrowWhenNoData(toThrow = IllegalStateException.class, startDescription = "Can't get:")
public class MyGetStepSupplier<T, S extends Iterable<T>> extends SequentialGetStepSupplier.GetListStepSupplier<
        MyTestContext,
        S, //тип Iterable/коллекции, который может быть обработан в рамках данного шага
        //и преобразован в результирующий не-модифицируемый объект java.util.List
        T,
        MyGetStepSupplier<T, S>>
{
    ...
    protected MyGetStepSupplier() {
        this(myTestContext -> {
            ...
            ///дальнейшие вычисления
            return //возврат КОЛЛЕКЦИИ или какого либо другого Iterable<T>,
            //которые будут преобразованы в не-модифицируемый объект java.util.List
        });
    }
 ...
 */
@ThrowWhenNoData(toThrow = IllegalStateException.class, startDescription = "Can't get:")
public class MyGetStepSupplier<T, M, S extends Iterable<T>> extends SequentialGetStepSupplier.GetListChainedStepSupplier<
        MyTestContext,
        S, //тип Iterable/коллекции, который может быть обработан в рамках данного шага
        //и преобразован в результирующий не-модифицируемый объект java.util.List
        M,
...
#### Объединение критериев в один логическим И
...
#### Объединение критериев в один инклюзивным ИЛИ
...
#### Объединение критериев в один исключающим ИЛИ
...
public class MyGetStepSupplier<T, M, P>
        ...
        //extends SequentialGetStepSupplier.GetListStepSupplier<>
        //extends SequentialGetStepSupplier.GetListChainedStepSupplier<>
...
public class ParameterPojo implements StepParameterPojo{ //интерфейс объектов, цель которых - собирать 
...
//Здесь описывается исключение, которое может быть выброшено, если в результате выполнения 
//построенного шага не было получено значение, соответствующее указанным критериям, и за указанное время.
//Указывается класс исключения, у которого есть публичный конструктор с параметрами типа
//String (текст сообщения), или String (текст сообщения) и Throwable (причина). 
// 
// В данном примере может быть выброшено IllegalStateException,
//с текстом Can't get: <описание получаемого значения и параметров шага>
//
//Аннотацию можно не указывать, т.к. по умолчанию может быть выброшено
// ru.tinkoff.qa.neptune.core.api.steps.NotPresentException с текстом "Not present: <текст>"
        ...
        //extends SequentialGetStepSupplier.GetArrayChainedStepSupplier<>
{
    ...
    //так же можжно делать аттачи, используя значения нестатических полей
    @CaptureOnSuccess(by = {ExampleCaptor.class, ExampleCaptor2.class})
    @CaptureOnFailure(by = ExampleCaptor3.class)
