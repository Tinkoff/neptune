...
[Критерии для получения подходящего http ответа](https://tinkoffcreditsystems.github.io/neptune/http.api/ru/tinkoff/qa/neptune/retrofit2/criteria/ResponseCriteria.html)
...
public class MyTest {
    ...
    public void myTest() {
        ...
        List<DtoObject> result = retrofit().get(
                ...
                //можно указать критерий, которому должно соответствовать тело ответа, т.e. 
                // оно должно содержать 2 объекта
...
public class MyTest {
    ...
    public void myTest() {
        ...
        List<DtoObject> result = retrofit().get(callIterable("Результирующий лист", //описываем то, 
                ...
                //можно указать критерий ожидаемого ответа, т.е. код статуса должен быть 200
                .responseCriteria(statusCode(200))
                //можно указать критерий, которому должен соответствовать элемент результирующей коллекции
...
public class MyTest {
    ...
    public void myTest() {
        ...
        DtoObject[] result = retrofit().get(callArray("Результирующий массив", //описываем то, 
                ...
                //можно указать критерий ожидаемого ответа, т.е. код статуса должен быть 200
                .responseCriteria(statusCode(200))
                //можно указать критерий, которому должен соответствовать элемент результирующего массива
                .criteria("Число элементов значения поля 'object' == 2", r -> r.getObject().size() == 2)
                //можно указать время на получение ожидаемого результата = 10 секунд
                //В течение этих 10 секунд запрос будет отправляться до тех пор,
                //пока не вернется ожидаемый ответ с ожидаемыми данными
